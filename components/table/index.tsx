...
export default function Table({ columns, data, itemsPerPage }: TableProps) {
  ...
  const colHeaders = columns.map(({ title, key }) => (
    <th tw="text-left py-5 text-common-grayText text-sm" key={key}>
      {title?.toUpperCase()}
    </th>
  ));
  ...
  const tabData = data.map((data, i) => {
    return (
      <tr
        key={`column${i}`}
        tw="border border-common-border border-t-0 border-l-0 border-r-0 hover:(bg-gray-50)"
      >
        {columns.map(({ render }, i2) => (
          <td tw="py-4 dark:(text-white)" key={`data${i}${i2}`}>
            {render(data, i)}
          </td>
        ))}
      </tr>
  ...
  return (
    <section tw="overflow-x-auto">
      <table tw="table-auto min-w-[48rem] w-full">
        <thead tw="border border-common-border border-t-0 border-l-0 border-r-0">
          <tr>{colHeaders}</tr>
        </thead>
